...
// Upsert provides a mock function with given fields: ctx, tenant, objectType, objectID, label
func (_m *LabelRepository) Upsert(ctx context.Context, tenant string, objectType model.LabelableObject, objectID string, label *model.Label) error {
	ret := _m.Called(ctx, tenant, objectType, objectID, label)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, string, model.LabelableObject, string, *model.Label) error); ok {
		r0 = rf(ctx, tenant, objectType, objectID, label)
	} else {
		r0 = ret.Error(0)
	}
